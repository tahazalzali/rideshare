...
import { metricsHandler } from '@rides/common';

async function bootstrap() {
  await initTracing('passenger-service');
  ...
  const doc = new DocumentBuilder().setTitle('Passenger Service').setVersion('1.0').build();
  ...
  app.getHttpAdapter().getInstance().get('/health', (req: any, res: any) => {
    ...
    res.json({ status: 'ok', service: 'passenger-service' });
  });
